    ...
    editor: (config, update) => (
      <div className="space-y-4">
         <div>
           ...
           <textarea className={`${styles.inputGlass} h-24 resize-none`} value={config.text} onChange={e => update({ text: e.target.value })} />
         </div>
         <button
          className={`w-full bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 py-2 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-50`}
          disabled={config.isGenerating}
...
export default function BuilderPage() {
  ...
  const [mockData] = useState({ data: { jmeno: "Jan Novák", role: "Administrátor", title: "Ukázkový Titulek", description: "Toto je data z kontextu." } });
              ...
              return (
                <div key={category}>
                  <div className={`text-[9px] font-bold tracking-[0.15em] uppercase mb-3 ${isAI ? "text-indigo-400 flex items-center gap-1" : "text-slate-500"}`}>
                    {isAI && <Sparkles size={10} />} {category}
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {items.map(([type, def]) => (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center p-3 rounded-xl border-[1px] ${styles.glassPanelHover} group relative overflow-hidden ${isAI ? 'bg-indigo-500/[0.05] border-indigo-500/30 hover:border-indigo-500/60 text-indigo-200' : 'bg-white/[0.02] border-indigo-500/20 hover:border-indigo-500/40 text-slate-300'}`}
                        >
